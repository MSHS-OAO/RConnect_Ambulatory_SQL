...
# test_data <- read_xlsx("utilization_test_data.xlsx")
# test_data <- test_data %>% filter(APPT_STATUS == "Arrived")%>% 
#   group_by(DEPARTMENT, PROV_NAME_WID, MRN, APPT_DTTM) %>% mutate(id = row_number()) %>% select(APPT_DTTM, APPT_DUR, id)
# data.hour.scheduled <- test_data

# noshow <- access %>% filter(APPT_STATUS == "No Show")
# same_day <- access %>% filter(APPT_STATUS %in% c("Canceled","Bumped","Rescheduled")) %>% filter(LEAD_DAYS == 0) %>% show_query()
# arrived <- access %>% filter(APPT_STATUS == "Arrived")
