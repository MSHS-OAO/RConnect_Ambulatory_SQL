...
utilization_query_scheduled <- glue("SELECT a.DEPARTMENT_ID, a.RESOURCES, a.PROVIDER, a.VISIT_METHOD, a.APPT_DATE_YEAR, a.APPT_MONTH_YEAR, a.APPT_YEAR, a.APPT_WEEK, a.APPT_DAY, a.HOLIDAY, a.APPT_DUR, a.APPT_DTTM, a.APPT_STATUS, a.MRN, a.VISIT_END_DTTM, a.ROOMIN_DTTM
...
data.hour.scheduled <- data.hour.scheduled[,c(names(data.hour.scheduled[,1:17]), timeOptionsHr) ]


data.hour.scheduled <- data.hour.scheduled %>% select(DEPARTMENT_ID, RESOURCES, PROVIDER, VISIT_METHOD, APPT_DATE_YEAR, APPT_MONTH_YEAR,              APPT_YEAR, APPT_WEEK, APPT_DAY, HOLIDAY, APPT_DUR, Appt.Start, Appt.End, APPT_DTTM, APPT_STATUS, MRN, all_of(timeOptionsHr)) 
...
data.hour.arrived <- data.hour.arrived[,c(names(data.hour.arrived[,1:18]), timeOptionsHr) ]



data.hour.arrived <- data.hour.arrived %>% select(DEPARTMENT_ID, RESOURCES, PROVIDER, VISIT_METHOD, APPT_DATE_YEAR, APPT_MONTH_YEAR, APPT_YEAR, APPT_WEEK, APPT_DAY, HOLIDAY, APPT_DUR, Appt.Start, Appt.End, APPT_DTTM,VISIT_END_DTTM, ROOMIN_DTTM, APPT_STATUS, MRN, all_of(timeOptionsHr))
...
utilization.data <- utilization.data %>% select(DEPARTMENT_ID, RESOURCES, PROVIDER, VISIT_METHOD, APPT_DATE_YEAR, APPT_MONTH_YEAR, APPT_YEAR, APPT_WEEK, APPT_DAY, HOLIDAY, APPT_DUR, Appt.Start, Appt.End, APPT_DTTM, APPT_STATUS, MRN, all_of(timeOptionsHr_filter))
...
get_values <- function(x,table_name){
  ...
  PROVIDER <- x$PROVIDER
  VISIT_METHOD <- x$VISIT_METHOD
  APPT_DATE_YEAR <- x$APPT_DATE_YEAR
  APPT_MONTH_YEAR <- x$APPT_MONTH_YEAR
  APPT_YEAR <- x$APPT_YEAR
  APPT_WEEK <- x$APPT_WEEK
  APPT_DAY <- x$APPT_DAY
  HOLIDAY <- x$HOLIDAY
  APPT_DUR <- x$APPT_DUR     
  Appt.Start <- x$Appt.Start
  Appt.End <- x$Appt.End
  APPT_DTTM <- x$APPT_DTTM
  APPT_STATUS <- x$APPT_STATUS
  MRN <-  x$MRN
  `07:00` <- x$`07:00`
  `08:00` <-  x$`08:00`
  `09:00` <-  x$`09:00`
...
special_character_columns <- c("RESOURCES", "PROVIDER", "VISIT_METHOD", "APPT_STATUS")
