...
#for (j in 1:686) {
  ...
  utilization_query_scheduled <- glue("SELECT a.DEPARTMENT_ID, a.RESOURCES, a.PROVIDER, a.VISIT_METHOD, a.APPT_DATE_YEAR, a.APPT_MONTH_YEAR, a.APPT_YEAR, a.APPT_WEEK, a.APPT_DAY, a.HOLIDAY, a.APPT_DUR, a.APPT_DTTM, a.APPT_STATUS, a.MRN, a.VISIT_END_DTTM, a.ROOMIN_DTTM, a.APPT_TYPE
  ...
  data.hour.scheduled <- utilization_data_raw %>% select(-VISIT_END_DTTM, -ROOMIN_DTTM) %>% filter(APPT_STATUS == "Arrived")
  ...
  data.hour.scheduled <- data.hour.scheduled %>% group_by(DEPARTMENT_ID, PROVIDER, MRN, APPT_DTTM) %>% filter(hour(Appt.Start) <= hour)
  data.hour.scheduled <- data.hour.scheduled %>% group_by(DEPARTMENT_ID, PROVIDER, MRN, APPT_DTTM) %>% filter(hour(Appt.End) >= hour)
  ...
  data.hour.scheduled <- 
                data.hour.scheduled %>%
                dplyr::group_by(DEPARTMENT_ID, RESOURCES, PROVIDER, VISIT_METHOD, APPT_DATE_YEAR, APPT_MONTH_YEAR, APPT_YEAR, APPT_WEEK, APPT_DAY, HOLIDAY, APPT_DUR, APPT_DTTM, APPT_STATUS, MRN, APPT_TYPE, Appt.Start, Appt.End, Appt.Date, hour) %>%
  ...
  data.hour.scheduled <- data.hour.scheduled %>% select(DEPARTMENT_ID, RESOURCES, PROVIDER, VISIT_METHOD, APPT_DATE_YEAR, APPT_MONTH_YEAR, APPT_YEAR, APPT_WEEK, APPT_DAY, HOLIDAY, APPT_DUR, Appt.Start, Appt.End, APPT_DTTM, APPT_STATUS, MRN, APPT_TYPE, all_of(timeOptionsHr)) 
  ...
  data.hour.arrived <- data.hour.arrived %>% group_by(DEPARTMENT_ID, PROVIDER, MRN, APPT_DTTM) %>% filter(hour(Appt.Start) <= hour)
  data.hour.arrived <- data.hour.arrived %>% group_by(DEPARTMENT_ID, PROVIDER, MRN, APPT_DTTM) %>% filter(hour(Appt.End) >= hour)
  ...
  data.hour.arrived <- data.hour.arrived %>%
    ...
    data.hour.arrived <- 
                data.hour.arrived %>%
                dplyr::group_by(DEPARTMENT_ID, RESOURCES, PROVIDER, VISIT_METHOD, APPT_DATE_YEAR, APPT_MONTH_YEAR, APPT_YEAR, APPT_WEEK, APPT_DAY, HOLIDAY, APPT_DUR, APPT_DTTM, APPT_STATUS, MRN, APPT_TYPE, Appt.Start, Appt.End, hour) %>%
                dplyr::mutate(n = dplyr::n()) %>%
                filter(n >=1) %>%
                  select(-n)%>%
                ungroup()
  ...
  data.hour.arrived <- data.hour.arrived[,c(names(data.hour.arrived[,1:19]), timeOptionsHr) ]
  
  
  
  data.hour.arrived <- data.hour.arrived %>% select(DEPARTMENT_ID, RESOURCES, PROVIDER, VISIT_METHOD, APPT_DATE_YEAR, APPT_MONTH_YEAR, APPT_YEAR, APPT_WEEK, APPT_DAY, HOLIDAY, APPT_DUR, Appt.Start, Appt.End, APPT_DTTM,VISIT_END_DTTM, ROOMIN_DTTM, APPT_STATUS, MRN, APPT_TYPE, all_of(timeOptionsHr))
  ...
  utilization.data <- utilization.data %>% select(DEPARTMENT_ID, RESOURCES, PROVIDER, VISIT_METHOD, APPT_DATE_YEAR, APPT_MONTH_YEAR, APPT_YEAR, APPT_WEEK, APPT_DAY, HOLIDAY, APPT_DUR, Appt.Start, Appt.End, APPT_DTTM, APPT_STATUS, MRN, sum, util_type, APPT_TYPE, all_of(timeOptionsHr_filter))
  ...
  get_values <- function(x,table_name){
    ...
    values <- glue("INTO \"{table_name}\" (DEPARTMENT_ID, 
                   ...
                   VALUES('{DEPARTMENT_ID}', '{RESOURCES}','{PROVIDER}',
  ...
  utilization.data <- utilization.data %>%
    mutate(DEPARTMENT_ID = as.numeric(DEPARTMENT_ID),
           ...
           select(DEPARTMENT_ID, RESOURCES, PROVIDER, 
  ...
  special_character_columns <- c("RESOURCES", "PROVIDER", "VISIT_METHOD", "APPT_STATUS", "APPT_TYPE")
