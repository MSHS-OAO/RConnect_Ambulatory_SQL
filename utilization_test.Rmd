...
utilization_query_scheduled <- glue("SELECT CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, RESOURCES, PROVIDER, VISIT_METHOD, APPT_DATE_YEAR, APPT_MONTH_YEAR, APPT_YEAR, APPT_WEEK, APPT_DAY, HOLIDAY, APPT_DUR, APPT_DTTM, APPT_STATUS, MRN, VISIT_END_DTTM, ROOMIN_DTTM
...
data.hour.scheduled <- utilization_data_raw %>% select(-VISIT_END_DTTM, -ROOMIN_DTTM)
...
data.hour.scheduled <- data.hour.scheduled %>% pivot_longer(all_of(hours), names_to = "hour", values_to = "min") %>% mutate(hour = as.numeric(hour))
...
# data.hour.arrived  <- data.hour.arrived  %>% mutate(actual.visit.dur = round(difftime(VISIT_END_DTTM, ROOMIN_DTTM, units = "mins")))
# 
# ########### Analysis of % of visits with actual visit start and end times ############
# data.hour.arrived  <- data.hour.arrived  %>% mutate(Appt.Start = format(strptime(as.ITime(ROOMIN_DTTM), "%H:%M:%S"),'%H:%M:%S'))
# data.hour.arrived  <- data.hour.arrived  %>% mutate(Appt.Start = as.POSIXct(Appt.Start, format = "%H:%M"))
# data.hour.arrived  <- data.hour.arrived  %>% mutate(Appt.End = as.POSIXct(Appt.Start + actual.visit.dur, format = "%H:%M"))


##Fill new columns with NA
data.hour.arrived[, hours] <- NA

data.hour.arrived <- data.hour.arrived %>% pivot_longer(all_of(hours), names_to = "hour", values_to = "min") %>% mutate(hour = as.numeric(hour))
...
# function to convert the record of data frame into statement
...
special_character_columns <- c("CAMPUS", "CAMPUS_SPECIALTY", "DEPARTMENT", "RESOURCES", "PROVIDER", "VISIT_METHOD", "APPT_STATUS")
