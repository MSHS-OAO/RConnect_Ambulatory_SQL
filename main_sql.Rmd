...
conn <- dbPool(drv = odbc::odbc(), dsn = "OAO Cloud DB", timeout = 30)

drop_access_query <- glue("DROP VIEW AMBULATORY_ACCESS")

date_1 <- "2021-01-01"
date_2 <- Sys.Date() - 1
reg_exp <- "\\[(.*?)\\]"

access_query <- glue("CREATE View AMBULATORY_ACCESS AS
SELECT i.*, 
