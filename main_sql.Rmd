...
### Maping Table
ambulatory_mapping_drop <- glue("DROP TABLE AMBULATORY_MAPPING_TEST")
ambulatory_mapping_query <- glue("CREATE TABLE AMBULATORY_MAPPING_TEST AS
...
ambulatory_mapping_index <- glue("CREATE index amb_mapping_index_test on AMBULATORY_MAPPING_TEST (CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, DEPARTMENT_ID)")

ambulatory_mapping_grant <- glue("GRANT SELECT on AMBULATORY_MAPPING_TEST to kweons01")

###Provider mapping table
ambulatory_provider_mapping <- glue("CREATE TABLE AMBULATORY_PROVIDER_MAPPING_TEST AS
...
drop_ambulatory_provider_mapping <- glue("DROP TABLE AMBULATORY_PROVIDER_MAPPING_TEST")

drop_access_query <- glue("DROP TABLE AMBULATORY_ACCESS_TEST")


###Main Ambulatory Table
access_query <- glue("CREATE TABLE AMBULATORY_ACCESS_TEST AS
...
(e.CHECKIN_DTTM - e.APPT_DTTM)*60*24 as DTTMTOCHECKIN
FROM(
...
FROM MV_DM_PATIENT_ACCESS a
        WHERE a.CONTACT_DATE BETWEEN TO_DATE('{date_1} 00:00:00', 'YYYY-MM-DD HH24:MI:SS')
                    ...
                    LEFT JOIN AMBULATORY_MAPPING_TEST i on d.DEPARTMENT_ID = i.DEPARTMENT_ID
                    LEFT JOIN AMBULATORY_SCHED_METHOD j on d.SCHED_METHOD_TRIMMED = LOWER(j.SCHED_METHOD)
                    ) c
                    )d
                    ) e")
access_index <- glue("CREATE INDEX access_index on AMBULATORY_ACCESS_TEST (APPT_DATE_YEAR, DEPARTMENT)")
access_index_npr <- glue("create index npr_index on AMBULATORY_ACCESS_TEST (APPT_MADE_DATE_YEAR, DEPARTMENT)")
access_permission <- glue("GRANT SELECT ON AMBULATORY_ACCESS_TEST TO kweons01")
access_permission_dev <- glue("GRANT SELECT ON AMBULATORY_ACCESS_TEST TO OAO_DEVELOPMENT")
access_permission_prod <- glue("GRANT SELECT ON AMBULATORY_ACCESS_TEST TO OAO_PRODUCTION")

bin_roomin <- glue("begin bin_hist('AMBULATORY_ACCESS_TEST', 'CHECKINTOROOMIN', 'bin_roomin', 30, 480, 0); end;")

bin_cycle <- glue("begin bin_hist('AMBULATORY_ACCESS_TEST', 'CYCLETIME', 'bin_cycle', 30, 480, 0); end;")

bin_roomin_visit_end <- glue("begin bin_hist('AMBULATORY_ACCESS_TEST', 'ROOMINTOVISITEND', 'bin_roomin_visit_end', 30, 480, 0); end;")

bin_dttm_checkin <- glue("begin bin_hist('AMBULATORY_ACCESS_TEST', 'DTTMTOCHECKIN', 'bin_dttm_checkin', 5, 60, -60); end;")

###Table specifically for global filters
drop_ambulatory_filters_query <- glue("DROP TABLE AMBULATORY_FILTERS_TEST")
ambulatory_filters_query <- glue("CREATE TABLE AMBULATORY_FILTERS_TEST AS
SELECT DISTINCT CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, PROVIDER, RESOURCES, VISIT_METHOD, APPT_TYPE FROM AMBULATORY_ACCESS_TEST")


## Slot Table
drop_slot_query <- glue("DROP TABLE AMBULATORY_SLOT_TEST")
slot_query <- glue("CREATE TABLE AMBULATORY_SLOT_TEST AS
...
FROM (
    ...
    LEFT JOIN AMBULATORY_MAPPING_TEST c on b.DEPARTMENT_ID = c.DEPARTMENT_ID  
    LEFT JOIN SUBTRACT_DAYS f on b.APPT_DAY = f.WEEKDAY")
    
    slot_index <- glue("CREATE index slot_index on AMBULATORY_SLOT_TEST (APPT_DATE_YEAR, DEPARTMENT_NAME)")



###Population View
drop_population_query <- glue("DROP VIEW AMBULATORY_POPULATION_TEST")
population_query <- glue("CREATE VIEW AMBULATORY_POPULATION_TEST AS 
SELECT g.*
FROM(
SELECT e.*,
CASE 
...
population_grant <- glue("GRANT SELECT ON AMBULATORY_POPULATION_TEST to KWEONS01")
...
###Query Execution
  tryCatch({
        ...
        dbBegin(conn)
            if(dbExistsTable(conn, "AMBULATORY_MAPPING_TEST")){
        ...
        if(dbExistsTable(conn, "AMBULATORY_PROVIDER_MAPPING_TEST")){
          dbExecute(conn, drop_ambulatory_provider_mapping) 
         }
         dbExecute(conn, ambulatory_provider_mapping)

        
        if(dbExistsTable(conn, "AMBULATORY_ACCESS_TEST")){
        ...
        dbExecute(conn, bin_roomin)
        dbExecute(conn, bin_cycle)
        dbExecute(conn, bin_roomin_visit_end)
        dbExecute(conn, bin_dttm_checkin)
        

        
        if(dbExistsTable(conn, "AMBULATORY_FILTERS_TEST")){
        ...
        # if(dbExistsTable(conn, "AMBULATORY_POPULATION_TEST")){
...
access_summary_drop <- glue("DROP TABLE AMBULATORY_ACCESS_TEST_SUMMARY_TABLE_TEST")
access_summary_query <- glue("CREATE TABLE AMBULATORY_ACCESS_TEST_SUMMARY_TABLE_TEST AS
SELECT
    CAMPUS,
    CAMPUS_SPECIALTY,
    DEPARTMENT,
...
FROM (VILLEA04.AMBULATORY_ACCESS_TEST)
GROUP BY
    CAMPUS,
    CAMPUS_SPECIALTY,
    DEPARTMENT,
    ...
    access_summary_index <- glue("create INDEX access_index_summary_test on AMBULATORY_ACCESS_SUMMARY_TABLE_TEST (APPT_DATE_YEAR, DEPARTMENT)")


###Grouped NPR Table
access_npr_summary_drop <- glue("DROP TABLE AMBULATORY_ACCESS_NPR_SUMMARY_TABLE_TEST")
access_npr_summary <- glue("CREATE TABLE AMBULATORY_ACCESS_NPR_SUMMARY_TABLE_TEST AS
...
FROM(
        SELECT
            ...
            a.NEW_PT2,
            a.NEW_PT3
        FROM (VILLEA04.AMBULATORY_ACCESS_TEST) a
    ) b
GROUP BY
    b.CAMPUS,
    b.CAMPUS_SPECIALTY,
    ...
    npr_index_summary <- glue("create INDEX npr_index_summary on AMBULATORY_ACCESS_NPR_SUMMARY_TABLE_TEST (APPT_MADE_DATE_YEAR, DEPARTMENT)")
    
    pop_table_dev <- glue("CREATE TABLE AMBULATORY_POPULATION_TEST AS 
SELECT g.*
FROM(
SELECT e.*,
CASE 
...
FROM (VILLEA04.AMBULATORY_ACCESS_TEST) a 
    ...
    drop_pop_dev <- glue("DROP TABLE AMBULATORY_POPULATION_TEST")
    index_pop_dev <- glue("create INDEX pop_dev_index on AMBULATORY_POPULATION_TEST (APPT_DATE_YEAR, DEPARTMENT)")
...
## Grouped Table execution
  tryCatch({
        ...
        dbBegin(poolcon_upt)
            if(dbExistsTable(poolcon_upt, "AMBULATORY_ACCESS_SUMMARY_TABLE_TEST")){
          ...
          dbExecute(poolcon_upt, access_summary_index)
            if(dbExistsTable(poolcon_upt, "AMBULATORY_ACCESS_NPR_SUMMARY_TABLE_TEST")){
          ...
          dbExecute(poolcon_upt, npr_index_summary)
             if(dbExistsTable(poolcon_upt, "AMBULATORY_POPULATION_TEST")){
