...
access_query <- glue("CREATE TABLE AMBULATORY_ACCESS_test_test AS
SELECT d.*, CASE WHEN TO_CHAR(d.APPT_DTTM, 'HH24') >= 12 and TO_CHAR(d.APPT_DTTM, 'HH24') <= 17 THEN 'PM' 
     WHEN TO_CHAR(d.APPT_DTTM, 'HH24') < 12 Then 'AM'
     ELSE 'EVE' END AS AM_PM, nvl(d.NEW_PT3_RAW, 'ESTABLISHED') AS NEW_PT3, nvl(d.SCHEDULE_GROUPING, 'Other') AS SCHEDULE_GROUPING_MAPPED,
nvl(d.CAMPUS_NEW, 'NA') AS CAMPUS, nvl(d.CAMPUS_SPECIALTY_NEW, 'NA') AS CAMPUS_SPECIALTY,
CASE d.ACCESS_CENTER WHEN 'Y' THEN 'Access Center' ELSE d.APPT_SOURCE_MAP_OTHER END AS Appt_Source_New,
least(greatest(floor(d.DTTMTOCHECKIN / 5)*5, -65), 60) as bin_checkin,
least(greatest(floor(d.CHECKINTOROOMIN / 30)*30, 0), 480) as bin_roomin, 
least(greatest(floor(d.CYCLETIME / 30)*30, 0), 480) as bin_cycle,
least(greatest(floor(((d.min_of_checkout_visit - d.ROOMIN_DTTM)*24*60) / 30)*30, 0), 480) as bin_roomin_visit_end,
...
EXTRACT(year from c.APPT_DTTM) Appt_Year,
                       ...
                       CONCAT('Q',TO_CHAR(c.APPT_MADE_DTTM, 'Q')) AS APPT_MADE_QUARTER,
                        ...
                        (c.CHECKIN_DTTM - c.APPT_DTTM)*60*24 as DTTMTOCHECKIN,
                        c.APPT_DATE_YEAR - c.DAYS_SUBTRACT AS APPT_WEEK, NVL(c.APPT_SOURCE_MAP, 'Other') AS APPT_SOURCE_MAP_OTHER
    FROM(
     SELECT i.CAMPUS AS CAMPUS_NEW, i.CAMPUS_SPECIALTY AS CAMPUS_SPECIALTY_NEW, i.DEPARTMENT,a.*, b.holiday, f.DAYS_SUBTRACT, h.APPT_SOURCE_NEW AS APPT_SOURCE_MAP, j.SCHEDULE_GROUPING
    FROM(
 SELECT PROV_NAME_WID, PROV_ID, DEPARTMENT_ID AS Department_ID, REFERRING_PROV_NAME_WID AS Refferring_Provider, CYCLE_TIME_MINUTES AS cycleTime,
                             MRN, PAT_NAME AS Patient_Name, ZIP_CODE, BIRTH_DATE, FINCLASS AS Coverage,
                             APPT_MADE_DTTM, APPT_DTTM, PRC_NAME AS APPT_TYPE, APPT_LENGTH AS APPT_DUR, DERIVED_STATUS_DESC AS APPT_STATUS,
                             APPT_CANC_DTTM AS APPT_CANCEL_DTTM, CANCEL_REASON_NAME AS CANCEL_REASON,
                             SIGNIN_DTTM, PAGED_DTTM, CHECKIN_DTTM, ARVL_LIST_REMOVE_DTTM AS ARRIVAL_REMOVE_DTTM,
                             ROOMED_DTTM AS ROOMIN_DTTM, FIRST_ROOM_ASSIGN_DTTM AS ROOM_ASSIGNED_DTTM, 
                             PHYS_ENTER_DTTM AS PROVIDERIN_DTTM, VISIT_END_DTTM, CHECKOUT_DTTM,
                             TIME_IN_ROOM_MINUTES AS TIMEIN_ROOM, CYCLE_TIME_MINUTES AS CYCLE_TIME, LOS_NAME AS CLASS_PT, LOS_CODE,
                             DEP_RPT_GRP_THIRTYONE AS CADENCE, APPT_ENTRY_USER_NAME_WID AS APPT_SOURCE, 
                             ACCESS_CENTER_SCHEDULED_YN AS ACCESS_CENTER, VISIT_METHOD, VISIT_PROV_STAFF_RESOURCE_C,
                             PRIMARY_DX_CODE, ENC_CLOSED_CHARGE_STATUS, Y_ENC_COSIGN_TIME, Y_ENC_CLOSE_TIME, Y_ENC_OPEN_TIME, NPI, VISIT_GROUP_NUM AS NEW_PT, 
                             PAT_ENC_CSN_ID, VISITPLAN, ATTRIB_BILL_AREA,
                             TRIM(LOWER(SCHED_METHOD)) AS SCHED_METHOD_TRIMMED,
                             trunc(APPT_DTTM) AS Appt_Date_Year,
                             TO_CHAR(APPT_DTTM, 'DY') AS APPT_DAY,
                             TRIM(TRAILING FROM REGEXP_REPLACE(PROV_NAME_WID, '{reg_exp}', '')) AS Provider,
                             CASE VISIT_PROV_STAFF_RESOURCE_C WHEN 1 THEN 'Provider' ELSE 'Resource' END AS Resources,
                             trunc(APPT_MADE_DTTM) AS APPT_MADE_DATE_YEAR,
                             OVERBOOKED_YN
FROM MV_DM_PATIENT_ACCESS 
        WHERE CONTACT_DATE BETWEEN TO_DATE('{date_1} 00:00:00', 'YYYY-MM-DD HH24:MI:SS')
                        AND TO_DATE('{date_2} 23:59:59', 'YYYY-MM-DD HH24:MI:SS')
                             OR APPT_MADE_DTTM BETWEEN TO_DATE('{date_1} 00:00:00', 'YYYY-MM-DD HH24:MI:SS')
                        AND TO_DATE('{date_2} 23:59:59', 'YYYY-MM-DD HH24:MI:SS')
                    ) a
                    LEFT JOIN holidays b on a.Appt_Date_Year = b.dates
                    LEFT JOIN SUBTRACT_DAYS f on a.APPT_DAY = f.WEEKDAY
                    LEFT JOIN AMBULATORY_APPT_SOURCE h on a.APPT_SOURCE = h.APPT_SOURCE
                    LEFT JOIN AMBULATORY_MAPPING i on a.DEPARTMENT_ID = i.DEPARTMENT_ID
                    LEFT JOIN AMBULATORY_SCHED_METHOD j on a.SCHED_METHOD_TRIMMED = LOWER(j.SCHED_METHOD)
                    ) c
                    ) d")
...
###Query Execution
  tryCatch({
        conn <- dbConnect(drv = odbc(), "OAO Cloud DB Production", timeout = 30)
        dbBegin(conn)
        ...
        #dbExecute(conn, "ALTER SESSION FORCE PARALLEL QUERY")
        dbExecute(conn, access_query)
        #dbExecute(conn, "ALTER SESSION DISABLE PARALLEL QUERY")
...
GROUP BY
    ...
    pop_table_dev <- glue("CREATE TABLE AMBULATORY_POPULATION_SUMMARY_TABLE AS 
SELECT g.*
FROM(
SELECT e.*,
CASE 
...
FROM (AMBULATORY_ACCESS) a 
    ...
    drop_pop_dev <- glue("DROP TABLE AMBULATORY_POPULATION_SUMMARY_TABLE")
    index_pop_dev <- glue("create INDEX pop_dev_index on AMBULATORY_POPULATION_SUMMARY_TABLE (APPT_DATE_YEAR, DEPARTMENT)")
  ...
  tryCatch({
        ...
        dbBegin(poolcon_upt)
          ...
          dbExecute(poolcon_upt, npr_index_summary)
             if(dbExistsTable(poolcon_upt, "AMBULATORY_POPULATION_SUMMARY_TABLE")){
...
```{r}
# add_new_bin_column <- glue("ALTER TABle AMBULATORY_ACCESS add bin_checkin number(5,0)")
# 
# update_access_table <- glue("UPDATE AMBULATORY_ACCESS set bin_checkin = least(greatest(floor(DTTMTOCHECKIN / 5)*5, -60), 60)")
# 
# tryCatch({
#         poolcon_upt <- dbConnect(drv = odbc(), "OAO Cloud DB Production", timeout = 30)
#         time_1 <- Sys.time()
#         dbBegin(poolcon_upt)
#             dbExecute(poolcon_upt, add_new_bin_column) 
#             dbExecute(poolcon_upt, update_access_table) 
#         dbCommit(poolcon_upt)
#         dbDisconnect(poolcon_upt)
#         time_2 <- Sys.time()
#         
