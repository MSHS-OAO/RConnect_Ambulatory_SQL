...
### Maping Table
ambulatory_mapping_drop <- glue("DROP TABLE AMBULATORY_MAPPING_TEST1")
ambulatory_mapping_query <- glue("CREATE TABLE AMBULATORY_MAPPING_TEST1 AS
...
ambulatory_mapping_index <- glue("CREATE index amb_mapping_index_test1 on AMBULATORY_MAPPING_TEST1 (CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, DEPARTMENT_ID)")
#ambulatory_mapping_grant <- glue("GRANT SELECT on AMBULATORY_MAPPING to kweons01")

###Provider mapping table
ambulatory_provider_mapping <- glue("CREATE TABLE AMBULATORY_PROVIDER_MAPPING_TEST1 AS
...
drop_ambulatory_provider_mapping <- glue("DROP TABLE AMBULATORY_PROVIDER_MAPPING_TEST1")

drop_access_query <- glue("DROP TABLE AMBULATORY_ACCESS_TEST1")


###Main Ambulatory Table
access_query <- glue("CREATE TABLE AMBULATORY_ACCESS_TEST1 AS
...
least(greatest(floor(DTTMTOCHECKIN / 5)*5, -60), 60) as bin_checkin,
least(greatest(floor(CHECKINTOROOMIN / 30)*30, 0), 480) as bin_roomin, 
least(greatest(floor(CYCLETIME / 30)*30, 0), 480) as bin_cycle,
least(greatest(floor(ROOMINTOVISITEND / 30)*30, 0), 480) as bin_roomin_visit_end
FROM(
...
(d.min_of_checkout_visit - d.ROOMIN_DTTM)*24*60 AS ROOMINTOVISITEND,
 (d.CHECKIN_DTTM - d.APPT_DTTM)*60*24 as DTTMTOCHECKIN
...
FROM MV_DM_PATIENT_ACCESS a
        WHERE a.CONTACT_DATE BETWEEN TO_DATE('{date_1} 00:00:00', 'YYYY-MM-DD HH24:MI:SS')
                    ...
                    LEFT JOIN AMBULATORY_MAPPING_TEST1 i on d.DEPARTMENT_ID = i.DEPARTMENT_ID
                    LEFT JOIN AMBULATORY_SCHED_METHOD j on d.SCHED_METHOD_TRIMMED = LOWER(j.SCHED_METHOD)
                    ) c
                    )d
                    ) e")

access_index <- glue("CREATE INDEX access_index_test1 on AMBULATORY_ACCESS_TEST1 (APPT_DATE_YEAR, DEPARTMENT)")
access_index_npr <- glue("create index npr_index_test1 on AMBULATORY_ACCESS_TEST1 (APPT_MADE_DATE_YEAR, DEPARTMENT)")
#access_permission <- glue("GRANT SELECT ON AMBULATORY_ACCESS TO kweons01")



###Table specifically for global filters
drop_ambulatory_filters_query <- glue("DROP TABLE AMBULATORY_FILTERS_TEST1")
ambulatory_filters_query <- glue("CREATE TABLE AMBULATORY_FILTERS_TEST1 AS
SELECT DISTINCT CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, PROVIDER, RESOURCES, VISIT_METHOD, APPT_TYPE FROM AMBULATORY_ACCESS_TEST1")


## Slot Table
drop_slot_query <- glue("DROP TABLE AMBULATORY_SLOT_TEST1")
slot_query <- glue("CREATE AMBULATORY_SLOT_TEST1 AS
...
FROM (
    ...
    LEFT JOIN AMBULATORY_MAPPING_TEST1 c on b.DEPARTMENT_ID = c.DEPARTMENT_ID  
    LEFT JOIN SUBTRACT_DAYS f on b.APPT_DAY = f.WEEKDAY")
    
    slot_index <- glue("CREATE index slot_index_test1 on AMBULATORY_SLOT_TEST1 (APPT_DATE_YEAR, DEPARTMENT_NAME)")



###Population View
drop_population_query <- glue("DROP VIEW AMBULATORY_POPULATION_TEST1")
population_query <- glue("CREATE VIEW AMBULATORY_POPULATION_TEST1 AS 
SELECT g.*
FROM(
SELECT e.*,
CASE 
...
FROM AMBULATORY_ACCESS_TEST1 a 
    ) b
...
###Query Execution
  tryCatch({
        ...
        dbBegin(conn)
            if(dbExistsTable(conn, "AMBULATORY_MAPPING_TEST1")){
        ...
        if(dbExistsTable(conn, "AMBULATORY_PROVIDER_MAPPING_TEST1")){
          dbExecute(conn, drop_ambulatory_provider_mapping)
         }
         dbExecute(conn, ambulatory_provider_mapping)
         start_time <- Sys.time()
         
        if(dbExistsTable(conn, "AMBULATORY_ACCESS_TEST1")){
        ...
        if(dbExistsTable(conn, "AMBULATORY_FILTERS_TEST1")){
        ...
        if(dbExistsTable(conn, "AMBULATORY_POPULATION_TEST1")){
...
access_summary_drop <- glue("DROP TABLE AMBULATORY_ACCESS_SUMMARY_TABLE_TEST1")
access_summary_query <- glue("CREATE TABLE AMBULATORY_ACCESS_SUMMARY_TABLE_TEST1 AS
SELECT
    CAMPUS,
    CAMPUS_SPECIALTY,
    DEPARTMENT,
...
GROUP BY
    ...
    access_summary_index <- glue("create INDEX access_index_summary_test1 on AMBULATORY_ACCESS_SUMMARY_TABLE_TEST1 (APPT_DATE_YEAR, DEPARTMENT)")


###Grouped NPR Table
access_npr_summary_drop <- glue("DROP TABLE AMBULATORY_ACCESS_NPR_SUMMARY_TABLE_TEST1")
access_npr_summary <- glue("CREATE TABLE AMBULATORY_ACCESS_NPR_SUMMARY_TABLE_TEST1 AS
...
GROUP BY
    ...
    npr_index_summary <- glue("create INDEX npr_index_summary_test1 on AMBULATORY_ACCESS_NPR_SUMMARY_TABLE_TEST1 (APPT_MADE_DATE_YEAR, DEPARTMENT)")
    
    pop_table_dev <- glue("CREATE TABLE AMBULATORY_POPULATION_SUMMARY_TABLE_TEST1 AS 
SELECT g.*
FROM(
SELECT e.*,
CASE 
...
FROM (AMBULATORY_ACCESS_TEST1) a 
    ...
    drop_pop_dev <- glue("DROP TABLE AMBULATORY_POPULATION_SUMMARY_TABLE_TEST1")
    index_pop_dev <- glue("create INDEX pop_dev_index_test1 on AMBULATORY_POPULATION_SUMMARY_TABLE_TEST1 (APPT_DATE_YEAR, DEPARTMENT)")
...
## Grouped Table execution
  tryCatch({
        ...
        dbBegin(poolcon_upt)
            if(dbExistsTable(poolcon_upt, "AMBULATORY_ACCESS_SUMMARY_TABLE_TEST1")){
          ...
          dbExecute(poolcon_upt, access_summary_index)
            if(dbExistsTable(poolcon_upt, "AMBULATORY_ACCESS_NPR_SUMMARY_TABLE_TEST1")){
          ...
          dbExecute(poolcon_upt, npr_index_summary)
             if(dbExistsTable(poolcon_upt, "AMBULATORY_POPULATION_SUMMARY_TABLE_TEST1")){
