...
EXTRACT(year from c.APPT_DTTM) Appt_Year,
    ...
    FROM(
     SELECT i.CAMPUS AS CAMPUS_NEW, i.CAMPUS_SPECIALTY, i.DEPARTMENT,d.*, b.holiday, f.DAYS_SUBTRACT, h.APPT_SOURCE_NEW AS APPT_SOURCE_MAP, j.SCHEDULE_GROUPING
    FROM(
...
access_index <- glue("CREATE INDEX access_index on AMBULATORY_ACCESS_TABLE (APPT_DATE_YEAR, DEPARTMENT)")
access_index_npr <- glue("create index npr_index on AMBULATORY_ACCESS_TABLE (APPT_MADE_DATE_YEAR, DEPARTMENT)")
access_permission <- glue("GRANT SELECT ON AMBULATORY_ACCESS_TABLE TO kweons01")
access_permission_dev <- glue("GRANT SELECT ON AMBULATORY_ACCESS_TABLE TO OAO_DEVELOPMENT")
