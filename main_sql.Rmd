...
ambulatory_mapping_index <- glue("CREATE index amb_mapping_index on AMBULATORY_ACCESS_TABLE (CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, DEPARTMENT_ID)")
...
SELECT c.*
    FROM(
     SELECT i.CAMPUS, i.CAMPUS_SECIALTY. i.DEPARTMENT,d.*, b.holiday, f.DAYS_SUBTRACT, h.APPT_SOURCE_NEW AS APPT_SOURCE_MAP 
    FROM(
 SELECT a.PROV_NAME_WID, a.DEPARTMENT_ID AS Department_ID, a.REFERRING_PROV_NAME_WID AS Refferring_Provider,
...
SELECT b.*, c.CAMPUS, c.CAMPUS_SPECIALTY, c.DEPARTMENT AS DEPARTMENT_NAME, b.APPT_DATE_YEAR - f.DAYS_SUBTRACT AS APPT_WEEK
FROM (
    SELECT
